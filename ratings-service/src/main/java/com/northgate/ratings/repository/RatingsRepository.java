...
public class RatingsRepository {
    ...
    public List<Rating> search(String namePattern, String sector) {
        ...
        StringBuilder sql = new StringBuilder("SELECT * FROM ratings WHERE issuer_name LIKE ?");
        args.add("%" + namePattern + "%");
        if (sector != null && !sector.isEmpty()) {
            sql.append(" AND sector = ?");
            args.add(sector);
        }
        sql.append(" ORDER BY issuer_name");
        return jdbc.query(sql.toString(), MAPPER, args.toArray());
    }

    public List<Rating> findByGrades(String gradeCsv) {
        List<Object> grades = new ArrayList<>();
        StringBuilder sql = new StringBuilder("SELECT * FROM ratings WHERE grade IN (");
        ...
        sql.append(')');
        return jdbc.query(sql.toString(), MAPPER, grades.toArray());
    }
